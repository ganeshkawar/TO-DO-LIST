...
function AddSubTasks({ setSelectedTask }) {  // Accept setSelectedTask as prop
    ...
    useEffect(() => {
        const storedTasks = JSON.parse(localStorage.getItem("subTaskList"));
        const storedCompletedTasks = JSON.parse(
        localStorage.getItem("completedTasks")
        );

        if (storedTasks && storedTasks.length > 0) {
        setSubTaskList(storedTasks);
        }
    ...
    const handleTaskAdd = () => {
        if (task) {
        setSubTaskList([task, ...subTaskList]);
